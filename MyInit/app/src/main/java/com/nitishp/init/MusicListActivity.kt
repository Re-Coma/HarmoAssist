...
class MusicListActivity :AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val spaceDecoration = RecyclerDecoration(1)

        val music = arrayListOf<Music>()
        for (i in 0..10) {
            music.add(Music("곡 제목", "곡 내용"))
        }
        recycler_view.apply {
            layoutManager = LinearLayoutManager(this@MusicListActivity)
            adapter = MusicAdapter(music) { music ->

                Toast.makeText(this@MusicListActivity, "${music.title} 선택됨", Toast.LENGTH_SHORT)
                    .show()
            }
    ...
    class MusicAdapter(val items :List<Music>,
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MusicViewHolder {
            ...
            view.setOnClickListener {
                ...
                if (mSelectedItems.get(itemPosition, false) || selected) {
                    mSelectedItems.put(itemPosition, false)
                    view.setBackgroundColor(Color.WHITE)
                    selected = false
                } else if(!mSelectedItems.get(itemPosition, false)) {
        ...
        override fun onBindViewHolder(holder: MusicViewHolder, position: Int) {
            ...
            if (mSelectedItems.get(position, false)) {
                ...
                selected =true
            } else if(mSelectedItems.get(position,  true)){
